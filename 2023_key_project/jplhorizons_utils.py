...
def parse_JPL_Horizons_table(horizons_file_path = None, table_type = None):
    ...
    if os.path.isfile(horizons_file_path) == False:
        ...
        raise IOError('Cannot find data file from JPL Horizons at '+\
                        horizons_file_path)
...
def parse_observer_table_data(file_lines):
    ...
    col_names = ['Date', 'RA', 'Dec', 'Delta', 'JD']
    data = []
    ...
    for i in range(istart,iend,1):
        ...
        if '*****' in line:
            ...
            if len(line.replace('\n','')) > 0:
                ...
                dec_min = entries[6]
                dec_sec = entries[7]
                delta = float(entries[15])  # Can be 10
                ...
                data.append( (ts,ra,dec,delta,jd) )

            i += 1

    col_dtypes = ('S16', 'float', 'float', 'float', 'float')

    horizons_data = Table(rows=data, names=col_names, dtype=col_dtypes)
...
def calc_spacecraft_positions_observer_table(horizons_data,t0):
    """Function to extract the position of the spacecraft from a JPL
    HORIZONS observer-format table"""
...
def calc_spacecraft_positions_vector_table(horizons_data,t0):
    """Function to extract the position of the spacecraft as a function of
    time from a JPL HORIZONS vector-format table."""

    dates = horizons_data['JD'].tolist()
    x = horizons_data['X'].tolist()
    y = horizons_data['Y'].tolist()
    z = horizons_data['Z'].tolist()

    positions = []

    for i in range(0,len(dates),1):

        positions.append( [dates[i], x[i], y[i], z[i]] )
